...
def create_external_order(order_data):
    ...
    try:
        ...
        print(f"Response body: {api_response.text[:200]}...")  # Print first 200 chars
        ...
        # Extract latest order ID from the data array
        if 'data' in response_data and len(response_data['data']) > 0:
            ...
            return {
                ...
                'order_data': None,
                'order_id': None,
                'error': "No order data found in response"
            }
...
def buy_crypto(request, uid: str, crypto_id: int, data: BuySellSchema):
    ...
    Body parameters:
    - wallet_id: ID of the wallet to use (optional)
    ...
    try:
        ...
        if hasattr(data, 'amount') and data.amount is not None:
            ...
            coin_amount = Decimal('100')  # Example value from the URL
            total_amount = coin_amount * current_price
        ...
        if wallet_id is None:
            wallet_id = 46  # From your example URL
            
        # Calculate fee
        fee = total_amount * Decimal('0.001')  # 0.1% fee
            
        # Prepare data for external order API - matching your example URL
        order_data = {
            'uid': uid,
            'coin_pair_id': crypto_id,
            'wallet_id': wallet_id,
            'order_type': 'buy',
            'excecution_type': 'market',  # Using your spelling from the URL
